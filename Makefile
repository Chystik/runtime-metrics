...
test-local:
	...
	./metricstest -test.v -test.run=^TestIteration$(iter)$$ -agent-binary-path=$(agent-bin) -binary-path=$(server-bin) -server-port=8080 -source-path=. -file-storage-path=./data.json
